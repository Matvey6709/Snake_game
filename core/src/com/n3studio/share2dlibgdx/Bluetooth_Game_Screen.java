...
public class Bluetooth_Game_Screen extends Сlassic_Game_Screen {
    ServerUpdate update;
    com.n3studio.share2dlibgdx.AnimationBluetooth animationBluetooth;
    PointUi pointUi;
...
//                game.bluetoothLoaded.restartGame();
        ...
        timeB = 0;
        stopGames = 0;
        game.loaded.toast(snake.NamePlayer);
    }
    ...
    public void checkExit(float delta) {
        if (startGame > 3) {
        } else {
            ...
            if (stopGames > 15) {
                ...
                System.gc();
//                game.bluetoothLoaded.restartGame();
                game.loaded.toast("К вам никто не подключился или произошла какая-то ошибка");
                game.bluetoothLoaded.stopT();
            }
        }
    }
...
//            game.bluetoothLoaded.restartGame();
    ...
    public void finishGame() {
        if (gg.indexOf("-stop-") <= 0) {
            ...
            player.setCords(gg);
            player.setLevel("3");
            player.setName(snake.NamePlayer);
            player.setAppleX(0);
            player.setAppleY(0);
